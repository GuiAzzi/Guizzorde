exports.jwGenresBR = [{
    "id": 1,
    "short_name": "act",
    "technical_name": "action",
    "translation": "Ação \u0026 Aventura",
    "slug": "acao-and-aventura"
}, {
    "id": 2,
    "short_name": "ani",
    "technical_name": "animation",
    "translation": "Animação",
    "slug": "animacao"
}, {
    "id": 3,
    "short_name": "cmy",
    "technical_name": "comedy",
    "translation": "Comédia",
    "slug": "comedia"
}, {
    "id": 4,
    "short_name": "crm",
    "technical_name": "crime",
    "translation": "Crime",
    "slug": "crime"
}, {
    "id": 5,
    "short_name": "doc",
    "technical_name": "documentation",
    "translation": "Documentário",
    "slug": "documentario"
}, {
    "id": 6,
    "short_name": "drm",
    "technical_name": "drama",
    "translation": "Drama",
    "slug": "drama"
}, {
    "id": 7,
    "short_name": "fnt",
    "technical_name": "fantasy",
    "translation": "Fantasia",
    "slug": "fantasia"
}, {
    "id": 8,
    "short_name": "hst",
    "technical_name": "history",
    "translation": "História",
    "slug": "historia"
}, {
    "id": 9,
    "short_name": "hrr",
    "technical_name": "horror",
    "translation": "Terror",
    "slug": "terror"
}, {
    "id": 10,
    "short_name": "fml",
    "technical_name": "family",
    "translation": "Família",
    "slug": "familia"
}, {
    "id": 11,
    "short_name": "msc",
    "technical_name": "music",
    "translation": "Música \u0026 Musical",
    "slug": "musica-and-musical"
}, {
    "id": 12,
    "short_name": "trl",
    "technical_name": "thriller",
    "translation": "Mistério \u0026 Thriller",
    "slug": "misterio-and-thriller"
}, {
    "id": 13,
    "short_name": "rma",
    "technical_name": "romance",
    "translation": "Romance",
    "slug": "romance"
}, {
    "id": 14,
    "short_name": "scf",
    "technical_name": "scifi",
    "translation": "Ficção Científica",
    "slug": "ficcao-cientifica"
}, {
    "id": 15,
    "short_name": "spt",
    "technical_name": "sport",
    "translation": "Esporte \u0026 Fitness",
    "slug": "esporte-and-fitness"
}, {
    "id": 16,
    "short_name": "war",
    "technical_name": "war",
    "translation": "Guerra \u0026 Militar",
    "slug": "guerra-and-militar"
}, {
    "id": 17,
    "short_name": "wsn",
    "technical_name": "western",
    "translation": "Western",
    "slug": "western"
}, {
    "id": 23,
    "short_name": "rly",
    "technical_name": "reality",
    "translation": "Reality TV",
    "slug": "reality-tv"
}, {
    "id": 18,
    "short_name": "eur",
    "technical_name": "european",
    "translation": "Made in Europe",
    "slug": "made-in-europe"
}]

exports.jwGenresEN = [{
    "id": 1,
    "short_name": "act",
    "technical_name": "action",
    "translation": "Action \u0026 Adventure",
    "slug": "action-and-adventure"
},
{
    "id": 2,
    "short_name": "ani",
    "technical_name": "animation",
    "translation": "Animation",
    "slug": "animation"
},
{
    "id": 3,
    "short_name": "cmy",
    "technical_name": "comedy",
    "translation": "Comedy",
    "slug": "comedy"
},
{
    "id": 4,
    "short_name": "crm",
    "technical_name": "crime",
    "translation": "Crime",
    "slug": "crime"
},
{
    "id": 5,
    "short_name": "doc",
    "technical_name": "documentation",
    "translation": "Documentary",
    "slug": "documentary"
},
{
    "id": 6,
    "short_name": "drm",
    "technical_name": "drama",
    "translation": "Drama",
    "slug": "drama"
},
{
    "id": 7,
    "short_name": "fnt",
    "technical_name": "fantasy",
    "translation": "Fantasy",
    "slug": "fantasy"
},
{
    "id": 8,
    "short_name": "hst",
    "technical_name": "history",
    "translation": "History",
    "slug": "history"
},
{
    "id": 9,
    "short_name": "hrr",
    "technical_name": "horror",
    "translation": "Horror",
    "slug": "horror"
},
{
    "id": 10,
    "short_name": "fml",
    "technical_name": "family",
    "translation": "Kids \u0026 Family",
    "slug": "kids-and-family"
},
{
    "id": 11,
    "short_name": "msc",
    "technical_name": "music",
    "translation": "Music \u0026 Musical",
    "slug": "music-and-musical"
}, {
    "id": 12,
    "short_name": "trl",
    "technical_name": "thriller",
    "translation": "Mystery \u0026 Thriller",
    "slug": "mystery-and-thriller"
}, {
    "id": 13,
    "short_name": "rma",
    "technical_name": "romance",
    "translation": "Romance",
    "slug": "romance"
}, {
    "id": 14,
    "short_name": "scf",
    "technical_name": "scifi",
    "translation": "Science-Fiction",
    "slug": "science-fiction"
}, {
    "id": 15,
    "short_name": "spt",
    "technical_name": "sport",
    "translation": "Sport",
    "slug": "sport"
}, {
    "id": 16,
    "short_name": "war",
    "technical_name": "war",
    "translation": "War \u0026 Military",
    "slug": "war-and-military"
}, {
    "id": 17,
    "short_name": "wsn",
    "technical_name": "western",
    "translation": "Western",
    "slug": "western"
}, {
    "id": 23,
    "short_name": "rly",
    "technical_name": "reality",
    "translation": "Reality TV",
    "slug": "reality-tv"
}, {
    "id": 18,
    "short_name": "eur",
    "technical_name": "european",
    "translation": "Made in Europe",
    "slug": "made-in-europe"
}
]

exports.jwProvidersBR = [{
    "id": 8,
    "technical_name": "netflix",
    "short_name": "nfx",
    "clear_name": "Netflix",
    "priority": 10,
    "display_priority": 1,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/207360008/{profile}",
    "icon_blur_hash": "KVBJXFjt1^jto1a|A;a|,Z",
    "slug": "netflix",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.netflix.ninja", "com.netflix.mediaclient", "com.netflix"],
            "platforms": ["fire_tv", "android_tv", "tvos"],
            "path_template": "www.netflix.com/watch/%DEEPLINK%",
            "extras": {
                "S.source": "30"
            }
        }],
        "packages": {
            "android_tv": "com.netflix.ninja",
            "fire_tv": "com.netflix.ninja",
            "tvos": "com.netflix.ninja",
            "tizenos": "",
            "webos": ""
        }
    }
},
{
    "id": 167,
    "technical_name": "clarovideo",
    "short_name": "clv",
    "clear_name": "Claro video",
    "priority": 5,
    "display_priority": 2,
    "monetization_types": ["free", "rent", "flatrate"],
    "icon_url": "/icon/9899714/{profile}",
    "icon_blur_hash": "KPRo,4-pt,?HWBRj*JR*R5",
    "slug": "claro-video",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["claro"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "claro",
            "tizenos": "",
            "webos": ""
        }
    }
},
{
    "id": 119,
    "technical_name": "amazonprimevideo",
    "short_name": "prv",
    "clear_name": "Amazon Prime Video",
    "priority": 5,
    "display_priority": 3,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/52449861/{profile}",
    "icon_blur_hash": "KMQAQ@%M~Ux]WCjF~qj[IU",
    "slug": "amazon-prime-video",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }, {
            "scheme": "",
            "packages": ["aiv"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "aiv",
            "tizenos": "",
            "webos": ""
        }
    }
},
{
    "id": 47,
    "technical_name": "looke",
    "short_name": "lke",
    "clear_name": "Looke",
    "priority": 5,
    "display_priority": 4,
    "monetization_types": ["rent", "buy", "flatrate"],
    "icon_url": "/icon/755940/{profile}",
    "icon_blur_hash": "KW7e=Mj]Muflj[j?Dgayxu",
    "slug": "looke",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["hello_world"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "hello_world",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
},
{
    "id": 31,
    "technical_name": "hbogo",
    "short_name": "hbg",
    "clear_name": "HBO Go",
    "priority": 5,
    "display_priority": 5,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/614494/{profile}",
    "icon_blur_hash": "KLCGfHax4T.7azM|4mj[%g",
    "slug": "hbo-go",
    "data": {
        "deeplink_data": [{
            "scheme": "hbogo",
            "packages": ["com.hbo.go"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "%DEEPLINK%",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.hbogo"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.hbo.go",
            "fire_tv": "com.hbo.go",
            "tvos": "com.hbogo",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 2,
    "technical_name": "itunes",
    "short_name": "itu",
    "clear_name": "Apple iTunes",
    "priority": 5,
    "display_priority": 6,
    "monetization_types": ["rent", "buy"],
    "icon_url": "/icon/190848813/{profile}",
    "icon_blur_hash": "KSA^OK%MIUoffQay00M{%M",
    "slug": "apple-itunes",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 350,
    "technical_name": "appletvplus",
    "short_name": "atp",
    "clear_name": "Apple TV Plus",
    "priority": 5,
    "display_priority": 7,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/152862153/{profile}",
    "icon_blur_hash": "KH7w?1%MIUj[j[ay00IUxu",
    "slug": "apple-tv-plus",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["nice"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "nice",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 227,
    "technical_name": "telecineplay",
    "short_name": "tcp",
    "clear_name": "Telecine Play",
    "priority": 5,
    "display_priority": 8,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/176934433/{profile}",
    "icon_blur_hash": "KXR{cAt7%gXmayni?^WBMx",
    "slug": "telecine-play",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 307,
    "technical_name": "globoplay",
    "short_name": "gop",
    "clear_name": "Globo Play",
    "priority": 5,
    "display_priority": 9,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/136871678/{profile}",
    "icon_blur_hash": "KXSn]PxGqGxGa|WVqaWVoL",
    "slug": "globo-play",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["globoplay"],
            "platforms": ["android_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "globoplay",
            "fire_tv": "",
            "tvos": "globoplay",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 229,
    "technical_name": "foxplay",
    "short_name": "fpl",
    "clear_name": "Fox Play",
    "priority": 5,
    "display_priority": 10,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/92517046/{profile}",
    "icon_blur_hash": "KVS_o5s:qat7fkaeqGjZY5",
    "slug": "fox-play",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["foxplay"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "foxplay",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 228,
    "technical_name": "foxpremium",
    "short_name": "fpr",
    "clear_name": "Fox Premium",
    "priority": 5,
    "display_priority": 11,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/92161259/{profile}",
    "icon_blur_hash": "KcR]yjxsiIrFj[bbY+e.gh",
    "slug": "fox-premium",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["foxpremium"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "foxpremium",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 3,
    "technical_name": "play",
    "short_name": "ply",
    "clear_name": "Google Play Movies",
    "priority": 4,
    "display_priority": 12,
    "monetization_types": ["rent", "buy"],
    "icon_url": "/icon/169478387/{profile}",
    "icon_blur_hash": "KSQ,j#Ac~XK#JB$S%MxBMe",
    "slug": "google-play-movies",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.google.android.videos"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "www.youtube.com/watch?v=%DEEPLINK%",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.google.android.videos",
            "fire_tv": "com.google.android.videos",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 19,
    "technical_name": "netmovies",
    "short_name": "ntm",
    "clear_name": "NetMovies",
    "priority": 5,
    "display_priority": 13,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/446738/{profile}",
    "icon_blur_hash": "KWJQBbWB4U?0aysX8yof.7",
    "slug": "netmovies",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 68,
    "technical_name": "microsoft",
    "short_name": "msf",
    "clear_name": "Microsoft Store",
    "priority": 5,
    "display_priority": 14,
    "monetization_types": ["buy", "rent"],
    "icon_url": "/icon/820542/{profile}",
    "icon_blur_hash": "KYDBUIof0NWFjuWC4qWC?Y",
    "slug": "microsoft-store",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 11,
    "technical_name": "mubi",
    "short_name": "mbi",
    "clear_name": "Mubi",
    "priority": 11,
    "display_priority": 15,
    "monetization_types": ["flatrate", "rent"],
    "icon_url": "/icon/164970114/{profile}",
    "icon_blur_hash": "KF5rQpoiRib2axj@9DWCt7",
    "slug": "mubi",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["mubi"],
            "platforms": ["fire_tv", "android_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "mubi",
            "fire_tv": "mubi",
            "tvos": "mubi",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 283,
    "technical_name": "crunchyroll",
    "short_name": "cru",
    "clear_name": "Crunchyroll",
    "priority": 5,
    "display_priority": 16,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/127445869/{profile}",
    "icon_blur_hash": "KjSD4wozQRn$kCj]UvaKpJ",
    "slug": "crunchyroll",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["crunchyroll"],
            "platforms": ["fire_tv", "android_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "crunchyroll",
            "fire_tv": "crunchyroll",
            "tvos": "crunchyroll",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 190,
    "technical_name": "curiositystream",
    "short_name": "cts",
    "clear_name": "Curiosity Stream",
    "priority": 5,
    "display_priority": 17,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/48600036/{profile}",
    "icon_blur_hash": "KjS#6LoeiHn#fQkCU[f6em",
    "slug": "curiosity-stream",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["curiositystream"],
            "platforms": ["android_tv", "fire_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "curiositystream",
            "fire_tv": "curiositystream",
            "tvos": "curiositystream",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 464,
    "technical_name": "kocowa",
    "short_name": "koc",
    "clear_name": "Kocowa",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/207331738/{profile}",
    "icon_blur_hash": "KF8E3xxvRkxvogf900WCt7",
    "slug": "kocowa",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 447,
    "technical_name": "belasartesalacarte",
    "short_name": "bac",
    "clear_name": "Belas Artes à La Carte",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/202326287/{profile}",
    "icon_blur_hash": "KJCXjCbH66JTa|a|1zoKw^",
    "slug": "belas-artes-a-la-carte",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 465,
    "technical_name": "believe",
    "short_name": "bli",
    "clear_name": "Believe",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/207957738/{profile}",
    "icon_blur_hash": "KHPLgDN{HX-7jvkWL~aeu4",
    "slug": "believe",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 462,
    "technical_name": "vidplus",
    "short_name": "vid",
    "clear_name": "Vid Plus",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/207331558/{profile}",
    "icon_blur_hash": "KD8XdrteH_-paxkC8$RC%c",
    "slug": "vid-plus",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 457,
    "technical_name": "vixtv",
    "short_name": "vix",
    "clear_name": "VIX ",
    "priority": 5,
    "display_priority": 101,
    "monetization_types": ["ads"],
    "icon_url": "/icon/202926990/{profile}",
    "icon_blur_hash": "KI9QmxayIUofj[ay00j]%M",
    "slug": "vix",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 512,
    "technical_name": "tntgo",
    "short_name": "tgo",
    "clear_name": "TNTGo",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/209565607/{profile}",
    "icon_blur_hash": "KH8MN_soNaoLjta|1HS2xG",
    "slug": "tntgo",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 473,
    "technical_name": "revry",
    "short_name": "rvy",
    "clear_name": "Revry",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/208144706/{profile}",
    "icon_blur_hash": "KI8Mans;NYspjaa{0_R*xH",
    "slug": "revry",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 475,
    "technical_name": "docsville",
    "short_name": "dsv",
    "clear_name": "DOCSVILLE",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208148387/{profile}",
    "icon_blur_hash": "KFS=u7xuuPxuj[bHuOaenj",
    "slug": "docsville",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 477,
    "technical_name": "gospelplay",
    "short_name": "gos",
    "clear_name": "GOSPEL PLAY",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208171655/{profile}",
    "icon_blur_hash": "KUELyxV@M|b_t7Rj0LkCof",
    "slug": "gospel-play",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 478,
    "technical_name": "historyplay",
    "short_name": "hpl",
    "clear_name": "History Play",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208172686/{profile}",
    "icon_blur_hash": "KRC~34WC0$xsa|Rl5Aoe-T",
    "slug": "history-play",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 484,
    "technical_name": "nowonline",
    "short_name": "now",
    "clear_name": "NOW",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208853791/{profile}",
    "icon_blur_hash": "KF9tDee.IUxtjZRk00kCxu",
    "slug": "now",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 490,
    "technical_name": "arte1play",
    "short_name": "arp",
    "clear_name": "Arte1 Play ",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208860849/{profile}",
    "icon_blur_hash": "KUMg*pt6MdxuWBV@8wV@tl",
    "slug": "arte1-play",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 495,
    "technical_name": "cgoodtv",
    "short_name": "cgt",
    "clear_name": "CGood TV",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["free"],
    "icon_url": "/icon/208864843/{profile}",
    "icon_blur_hash": "KARbbt-_hg-wjbnOhfnOaK",
    "slug": "cgood-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 499,
    "technical_name": "oldflix",
    "short_name": "ofx",
    "clear_name": "Oldflix",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/209544344/{profile}",
    "icon_blur_hash": "KjNdamR*^+t7azs-~pM|M{",
    "slug": "oldflix",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 337,
    "technical_name": "disneyplus",
    "short_name": "dnp",
    "clear_name": "Disney Plus",
    "priority": 5,
    "display_priority": 1001,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/147638351/{profile}",
    "icon_blur_hash": "KQ5=;uogRNR1f5ohIUWBt6",
    "slug": "disney-plus",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.disney.disneyplus"],
            "platforms": ["android_tv"],
            "path_template": "disneyplus.com/%DEEPLINK%",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.disneyplus"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }, {
            "scheme": "https",
            "packages": ["com.disney.disneyplus"],
            "platforms": ["fire_tv"],
            "path_template": "disneyplus.com/%DEEPLINK%",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.disney.disneyplus",
            "fire_tv": "com.disney.disneyplus",
            "tvos": "com.disneyplus",
            "tizenos": "",
            "webos": ""
        }
    }
}]

exports.jwProvidersEN = [{
    "id": 8,
    "technical_name": "netflix",
    "short_name": "nfx",
    "clear_name": "Netflix",
    "priority": 10,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/207360008/{profile}",
    "icon_blur_hash": "KVBJXFjt1^jto1a|A;a|,Z",
    "slug": "netflix",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.netflix.ninja", "com.netflix.mediaclient", "com.netflix"],
            "platforms": ["fire_tv", "android_tv", "tvos"],
            "path_template": "www.netflix.com/watch/%DEEPLINK%",
            "extras": {
                "S.source": "30"
            }
        }],
        "packages": {
            "android_tv": "com.netflix.ninja",
            "fire_tv": "com.netflix.ninja",
            "tvos": "com.netflix.ninja",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 9,
    "technical_name": "amazonprime",
    "short_name": "amp",
    "clear_name": "Amazon Prime Video",
    "priority": 4,
    "display_priority": 2,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/52449539/{profile}",
    "icon_blur_hash": "KMQJ.ex]~VtlR*n%~qj[Mx",
    "slug": "amazon-prime-video",
    "data": {
        "deeplink_data": [{
            "scheme": "amzn",
            "packages": ["com.amazon.tv.launcher"],
            "platforms": ["fire_tv"],
            "path_template": "com.amazon.tv.launcher/detail?provider=aiv\u0026providerId=%DEEPLINK%",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }, {
            "scheme": "",
            "packages": ["com.aiv"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.tv.launcher",
            "tvos": "com.aiv",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 337,
    "technical_name": "disneyplus",
    "short_name": "dnp",
    "clear_name": "Disney Plus",
    "priority": 5,
    "display_priority": 3,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/147638351/{profile}",
    "icon_blur_hash": "KQ5=;uogRNR1f5ohIUWBt6",
    "slug": "disney-plus",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.disney.disneyplus"],
            "platforms": ["android_tv"],
            "path_template": "disneyplus.com/%DEEPLINK%",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.disneyplus"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }, {
            "scheme": "https",
            "packages": ["com.disney.disneyplus"],
            "platforms": ["fire_tv"],
            "path_template": "disneyplus.com/%DEEPLINK%",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.disney.disneyplus",
            "fire_tv": "com.disney.disneyplus",
            "tvos": "com.disneyplus",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 257,
    "technical_name": "fubotv",
    "short_name": "fuv",
    "clear_name": "fuboTV",
    "priority": 5,
    "display_priority": 4,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124248862/{profile}",
    "icon_blur_hash": "L~K@38j[1IayoLfQayfQaKf6kWfk",
    "slug": "fubotv",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["fubotv"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "fubotv",
            "fire_tv": "fubotv",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 350,
    "technical_name": "appletvplus",
    "short_name": "atp",
    "clear_name": "Apple TV Plus",
    "priority": 5,
    "display_priority": 5,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/152862153/{profile}",
    "icon_blur_hash": "KH7w?1%MIUj[j[ay00IUxu",
    "slug": "apple-tv-plus",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["nice"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "nice",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 2,
    "technical_name": "itunes",
    "short_name": "itu",
    "clear_name": "Apple iTunes",
    "priority": 5,
    "display_priority": 6,
    "monetization_types": ["rent", "buy"],
    "icon_url": "/icon/190848813/{profile}",
    "icon_blur_hash": "KSA^OK%MIUoffQay00M{%M",
    "slug": "apple-itunes",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 15,
    "technical_name": "hulu",
    "short_name": "hlu",
    "clear_name": "Hulu",
    "priority": 5,
    "display_priority": 7,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/116305230/{profile}",
    "icon_blur_hash": "KNBZ?7T1Rj%gR.ju0cWCt7",
    "slug": "hulu",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.hulu.livingroomplus"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "content_id": "%DEEPLINK%"
            }
        }, {
            "scheme": "",
            "packages": ["com.hulu"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.hulu.plus"],
            "platforms": ["fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.hulu.livingroomplus",
            "fire_tv": "com.hulu.plus",
            "tvos": "com.hulu",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 384,
    "technical_name": "hbomax",
    "short_name": "hbm",
    "clear_name": "HBO Max",
    "priority": 5,
    "display_priority": 8,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/182948653/{profile}",
    "icon_blur_hash": "KI9@PAog4m_2jtD%D}WTxv",
    "slug": "hbo-max",
    "data": {
        "deeplink_data": [{
            "scheme": "hbomax",
            "packages": ["hbomax"],
            "platforms": ["android_tv", "tvos"],
            "path_template": "hbomax",
            "extras": null
        }],
        "packages": {
            "android_tv": "hbomax",
            "fire_tv": "",
            "tvos": "hbomax",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 386,
    "technical_name": "peacocktv",
    "short_name": "pct",
    "clear_name": "Peacock",
    "priority": 5,
    "display_priority": 9,
    "monetization_types": ["ads"],
    "icon_url": "/icon/194173870/{profile}",
    "icon_blur_hash": "L55Ey,of00RjofayWBj[00WB_3t7",
    "slug": "peacock",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 387,
    "technical_name": "peacocktvpremium",
    "short_name": "pcp",
    "clear_name": "Peacock Premium",
    "priority": 5,
    "display_priority": 10,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/194173871/{profile}",
    "icon_blur_hash": "KWRysg%Mxu-;ayWB~qj[M{",
    "slug": "peacock-premium",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 10,
    "technical_name": "amazon",
    "short_name": "amz",
    "clear_name": "Amazon Video",
    "priority": 4,
    "display_priority": 11,
    "monetization_types": ["rent", "buy"],
    "icon_url": "/icon/430993/{profile}",
    "icon_blur_hash": "KYPG%2-p-;~AIoNHXU%2s+",
    "slug": "amazon-video",
    "data": {
        "deeplink_data": [{
            "scheme": "amzn",
            "packages": ["com.amazon.tv.launcher"],
            "platforms": ["fire_tv"],
            "path_template": "com.amazon.tv.launcher/detail?provider=aiv\u0026providerId=%DEEPLINK%",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.tv.launcher",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 3,
    "technical_name": "play",
    "short_name": "ply",
    "clear_name": "Google Play Movies",
    "priority": 4,
    "display_priority": 12,
    "monetization_types": ["rent", "buy"],
    "icon_url": "/icon/169478387/{profile}",
    "icon_blur_hash": "KSQ,j#Ac~XK#JB$S%MxBMe",
    "slug": "google-play-movies",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.google.android.videos"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "www.youtube.com/watch?v=%DEEPLINK%",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.google.android.videos",
            "fire_tv": "com.google.android.videos",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 192,
    "technical_name": "youtube",
    "short_name": "yot",
    "clear_name": "YouTube",
    "priority": 5,
    "display_priority": 13,
    "monetization_types": ["buy", "rent"],
    "icon_url": "/icon/59562423/{profile}",
    "icon_blur_hash": "K@SN$wxapcxaj@f*uikCa0",
    "slug": "youtube",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.google.android.youtube.tv"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "www.youtube.com/watch?v=%DEEPLINK%",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.google.android.youtube.tv",
            "fire_tv": "com.google.android.youtube.tv",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 188,
    "technical_name": "youtubered",
    "short_name": "ytr",
    "clear_name": "YouTube Premium",
    "priority": 5,
    "display_priority": 14,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/70189310/{profile}",
    "icon_blur_hash": "KgRx[mof*JrDf6o}*Jj[Q-",
    "slug": "youtube-premium",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 235,
    "technical_name": "youtubefree",
    "short_name": "yfr",
    "clear_name": "YouTube Free",
    "priority": 5,
    "display_priority": 15,
    "monetization_types": ["ads"],
    "icon_url": "/icon/113378150/{profile}",
    "icon_blur_hash": "L~LM#Lj[0Kayiwf6bvfkkqfkeTf6",
    "slug": "youtube-free",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 27,
    "technical_name": "hbonow",
    "short_name": "hbn",
    "clear_name": "HBO Now",
    "priority": 5,
    "display_priority": 16,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/203417770/{profile}",
    "icon_blur_hash": "KwFFpkayM{fQfQfQ00j[xu",
    "slug": "hbo-now",
    "data": {
        "deeplink_data": [{
            "scheme": "hbonow",
            "packages": ["com.hbo.hbonow"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "urn:hbo:feature:%DEEPLINK%",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.hbo"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.hbo.hbonow",
            "fire_tv": "com.hbo.hbonow",
            "tvos": "com.hbo",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 78,
    "technical_name": "cbs",
    "short_name": "cbs",
    "clear_name": "CBS",
    "priority": 5,
    "display_priority": 17,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/920372/{profile}",
    "icon_blur_hash": "KnG1Jnog0QogfQay4[az%J",
    "slug": "cbs",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.cbs"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "com.cbs",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 269,
    "technical_name": "funimationnow",
    "short_name": "fmn",
    "clear_name": "Funimation Now",
    "priority": 5,
    "display_priority": 18,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/188385942/{profile}",
    "icon_blur_hash": "KxK1hIWC02-:fQMy8|j[xt",
    "slug": "funimation-now",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 105,
    "technical_name": "fandangonow",
    "short_name": "fdg",
    "clear_name": "FandangoNOW",
    "priority": 5,
    "display_priority": 19,
    "monetization_types": ["rent", "buy"],
    "icon_url": "/icon/165634381/{profile}",
    "icon_blur_hash": "KmF;Wioz0zxuf+IpRjjYxa",
    "slug": "fandangonow",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["fandango"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "fandango",
            "fire_tv": "fandango",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 238,
    "technical_name": "imdb",
    "short_name": "ifd",
    "clear_name": "IMDb TV",
    "priority": 5,
    "display_priority": 20,
    "monetization_types": ["ads"],
    "icon_url": "/icon/134049674/{profile}",
    "icon_blur_hash": "KZQ8Enov?AJ8WEs-~3jbE7",
    "slug": "imdb-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 207,
    "technical_name": "rokuchannel",
    "short_name": "rkc",
    "clear_name": "The Roku Channel",
    "priority": 5,
    "display_priority": 21,
    "monetization_types": ["free", "ads"],
    "icon_url": "/icon/76972041/{profile}",
    "icon_blur_hash": "KaR:B3n+%LxuayWB_MozMy",
    "slug": "the-roku-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 212,
    "technical_name": "hoopla",
    "short_name": "hop",
    "clear_name": "Hoopla",
    "priority": 5,
    "display_priority": 22,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/80212823/{profile}",
    "icon_blur_hash": "KS91Y8b_Rku6kCj@4sjct6",
    "slug": "hoopla",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["hoopla"],
            "platforms": ["fire_tv", "android_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "hoopla",
            "fire_tv": "hoopla",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 83,
    "technical_name": "thecw",
    "short_name": "tcw",
    "clear_name": "The CW",
    "priority": 5,
    "display_priority": 23,
    "monetization_types": ["free", "flatrate"],
    "icon_url": "/icon/950814/{profile}",
    "icon_blur_hash": "KWB7t^kVI-cRj[j[0cayxb",
    "slug": "the-cw",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.cw"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "com.cw",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 206,
    "technical_name": "cwseed",
    "short_name": "cws",
    "clear_name": "CW Seed",
    "priority": 5,
    "display_priority": 24,
    "monetization_types": ["free"],
    "icon_url": "/icon/76259709/{profile}",
    "icon_blur_hash": "KYFjpCt7IUogWBj[0KNG%L",
    "slug": "cw-seed",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 7,
    "technical_name": "vudu",
    "short_name": "vdu",
    "clear_name": "Vudu",
    "priority": 9,
    "display_priority": 25,
    "monetization_types": ["buy", "rent"],
    "icon_url": "/icon/146383632/{profile}",
    "icon_blur_hash": "KN99+{WYM|kDj[fQ0Moext",
    "slug": "vudu",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["vudu"],
            "platforms": ["android_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "vudu",
            "fire_tv": "",
            "tvos": "vudu",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 332,
    "technical_name": "vudufree",
    "short_name": "vuf",
    "clear_name": "VUDU Free",
    "priority": 5,
    "display_priority": 26,
    "monetization_types": ["free"],
    "icon_url": "/icon/146383566/{profile}",
    "icon_blur_hash": "KNB5*qbID+IWfQxt0Mj[xt",
    "slug": "vudu-free",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 43,
    "technical_name": "starz",
    "short_name": "stz",
    "clear_name": "Starz",
    "priority": 5,
    "display_priority": 28,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/902247/{profile}",
    "icon_blur_hash": "KC7w.@WBD%xuWBay00WB-:",
    "slug": "starz",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.bydeluxe.d3.android.program.starz"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "%DEEPLINK%",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.starz"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.bydeluxe.d3.android.program.starz",
            "fire_tv": "com.bydeluxe.d3.android.program.starz",
            "tvos": "com.starz",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 258,
    "technical_name": "criterionchannel",
    "short_name": "crc",
    "clear_name": "Criterion Channel",
    "priority": 5,
    "display_priority": 29,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124249135/{profile}",
    "icon_blur_hash": "KC84i6%Mayj[offQ00IUj[",
    "slug": "criterion-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 37,
    "technical_name": "showtime",
    "short_name": "sho",
    "clear_name": "Showtime",
    "priority": 5,
    "display_priority": 30,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/677041/{profile}",
    "icon_blur_hash": "KYMhDkn%8woffRaeDOkCtR",
    "slug": "showtime",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.showtime"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "com.showtime",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 209,
    "technical_name": "pbs",
    "short_name": "pbs",
    "clear_name": "PBS",
    "priority": 5,
    "display_priority": 31,
    "monetization_types": ["free", "flatrate"],
    "icon_url": "/icon/78090545/{profile}",
    "icon_blur_hash": "L?J8V0j[00WBWBfQofayt7fQRjfQ",
    "slug": "pbs",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["pbs"],
            "platforms": ["tvos", "android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "pbs",
            "fire_tv": "pbs",
            "tvos": "pbs",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 177,
    "technical_name": "pantaflix",
    "short_name": "pfx",
    "clear_name": "Pantaflix",
    "priority": 5,
    "display_priority": 32,
    "monetization_types": ["rent"],
    "icon_url": "/icon/93814499/{profile}",
    "icon_blur_hash": "KJR2$zxr~M-*odt3^voKD;",
    "slug": "pantaflix",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["pantaflix"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "pantaflix",
            "fire_tv": "pantaflix",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 123,
    "technical_name": "fxnow",
    "short_name": "fxn",
    "clear_name": "FXNow",
    "priority": 5,
    "display_priority": 33,
    "monetization_types": ["free"],
    "icon_url": "/icon/4259277/{profile}",
    "icon_blur_hash": "LE71l@of4nWBoffQayay00WB_3of",
    "slug": "fxnow",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.fxnow"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "com.fxnow",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 73,
    "technical_name": "tubitv",
    "short_name": "tbv",
    "clear_name": "Tubi TV",
    "priority": 5,
    "display_priority": 34,
    "monetization_types": ["ads"],
    "icon_url": "/icon/104966985/{profile}",
    "icon_blur_hash": "LC7A3-ju0|fOnifQX8fk62fP=Ka#",
    "slug": "tubi-tv",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["tubitv"],
            "platforms": ["android_tv", "tvos", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "tubitv",
            "fire_tv": "tubitv",
            "tvos": "tubitv",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 226,
    "technical_name": "dcuniverse",
    "short_name": "dcu",
    "clear_name": "DC Universe",
    "priority": 5,
    "display_priority": 35,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/91378866/{profile}",
    "icon_blur_hash": "KQDS?-tR00RjWBRj00Rj-;",
    "slug": "dc-universe",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["dcuniverse"],
            "platforms": ["tvos", "android_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "dcuniverse",
            "fire_tv": "",
            "tvos": "dcuniverse",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 191,
    "technical_name": "kanopy",
    "short_name": "knp",
    "clear_name": "Kanopy",
    "priority": 5,
    "display_priority": 36,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/49764518/{profile}",
    "icon_blur_hash": "KG8gW[aiWBS$oej[0Kfiof",
    "slug": "kanopy",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 243,
    "technical_name": "comedycentral",
    "short_name": "com",
    "clear_name": "Comedy Central",
    "priority": 5,
    "display_priority": 37,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/112642053/{profile}",
    "icon_blur_hash": "KMCiZsWC0PxYjaWD9dt5t5",
    "slug": "comedy-central",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.ccnet"],
            "platforms": ["tvos", "android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.ccnet",
            "fire_tv": "com.ccnet",
            "tvos": "com.ccnet",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 68,
    "technical_name": "microsoft",
    "short_name": "msf",
    "clear_name": "Microsoft Store",
    "priority": 5,
    "display_priority": 38,
    "monetization_types": ["buy", "rent"],
    "icon_url": "/icon/820542/{profile}",
    "icon_blur_hash": "KYDBUIof0NWFjuWC4qWC?Y",
    "slug": "microsoft-store",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 283,
    "technical_name": "crunchyroll",
    "short_name": "cru",
    "clear_name": "Crunchyroll",
    "priority": 5,
    "display_priority": 39,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/127445869/{profile}",
    "icon_blur_hash": "KjSD4wozQRn$kCj]UvaKpJ",
    "slug": "crunchyroll",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["crunchyroll"],
            "platforms": ["fire_tv", "android_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "crunchyroll",
            "fire_tv": "crunchyroll",
            "tvos": "crunchyroll",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 309,
    "technical_name": "sunnxt",
    "short_name": "snx",
    "clear_name": "Sun Nxt",
    "priority": 5,
    "display_priority": 40,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/138047862/{profile}",
    "icon_blur_hash": "KYNa6~xurD#,i_Rj8wRjs:",
    "slug": "sun-nxt",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 279,
    "technical_name": "redbox",
    "short_name": "rbx",
    "clear_name": "Redbox",
    "priority": 5,
    "display_priority": 40,
    "monetization_types": ["buy", "rent"],
    "icon_url": "/icon/127420929/{profile}",
    "icon_blur_hash": "KsO.Xr$f8xOsa{WC8{o0of",
    "slug": "redbox",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 139,
    "technical_name": "maxgo",
    "short_name": "mxg",
    "clear_name": "Max Go",
    "priority": 5,
    "display_priority": 41,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/5160001/{profile}",
    "icon_blur_hash": "LRAvR1j@0jWWs-WWWDs-9yWW-Rs-",
    "slug": "max-go",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 148,
    "technical_name": "abcgo",
    "short_name": "abc",
    "clear_name": "ABC",
    "priority": 5,
    "display_priority": 43,
    "monetization_types": ["ads"],
    "icon_url": "/icon/5983493/{profile}",
    "icon_blur_hash": "KQDSgx%M9ZgOxte-00Rjxu",
    "slug": "abc",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 358,
    "technical_name": "directv",
    "short_name": "drv",
    "clear_name": "DIRECTV",
    "priority": 5,
    "display_priority": 44,
    "monetization_types": ["buy", "rent", "flatrate"],
    "icon_url": "/icon/158260222/{profile}",
    "icon_blur_hash": "KH6+CXt6MH9vWBr;iukCgO",
    "slug": "directv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 265,
    "technical_name": "tribecashortlist",
    "short_name": "tbs",
    "clear_name": "Tribeca Shortlist",
    "priority": 5,
    "display_priority": 45,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124874892/{profile}",
    "icon_blur_hash": "KABe^woN01oNfQayM{azxu",
    "slug": "tribeca-shortlist",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 12,
    "technical_name": "crackle",
    "short_name": "crk",
    "clear_name": "Crackle",
    "priority": 12,
    "display_priority": 46,
    "monetization_types": ["ads"],
    "icon_url": "/icon/176921838/{profile}",
    "icon_blur_hash": "KETNWmw0elxFg^f6dVfigh",
    "slug": "crackle",
    "data": {
        "deeplink_data": [{
            "scheme": "crackle",
            "packages": ["com.crackle.androidtv"],
            "platforms": ["android_tv"],
            "path_template": "Media/%DEEPLINK%",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.crackle.androidtv",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 162,
    "technical_name": "amctheatres",
    "short_name": "amt",
    "clear_name": "AMC Theatres",
    "priority": 5,
    "display_priority": 47,
    "monetization_types": ["cinema"],
    "icon_url": "/icon/7425739/{profile}",
    "icon_blur_hash": "KmL0k1so1as:fQay0{WVbv",
    "slug": "amc-theatres",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 80,
    "technical_name": "amc",
    "short_name": "amc",
    "clear_name": "AMC",
    "priority": 5,
    "display_priority": 48,
    "monetization_types": ["free"],
    "icon_url": "/icon/922527/{profile}",
    "icon_blur_hash": "KoEfTiayM{j[fQfQ00j[t7",
    "slug": "amc",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["amc"],
            "platforms": ["android_tv", "fire_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "amc",
            "fire_tv": "amc",
            "tvos": "amc",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 25,
    "technical_name": "fandor",
    "short_name": "fnd",
    "clear_name": "Fandor",
    "priority": 5,
    "display_priority": 49,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/554608/{profile}",
    "icon_blur_hash": "L_J8V0j[00WBj[j[fQayt7ayWBj[",
    "slug": "fandor",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 122,
    "technical_name": "hotstar",
    "short_name": "hst",
    "clear_name": "Hotstar",
    "priority": 5,
    "display_priority": 50,
    "monetization_types": ["free", "flatrate"],
    "icon_url": "/icon/240029443/{profile}",
    "icon_blur_hash": "L~Kx6pj[00ayoffQayfQt7fQRjfQ",
    "slug": "hotstar",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 190,
    "technical_name": "curiositystream",
    "short_name": "cts",
    "clear_name": "Curiosity Stream",
    "priority": 5,
    "display_priority": 50,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/48600036/{profile}",
    "icon_blur_hash": "KjS#6LoeiHn#fQkCU[f6em",
    "slug": "curiosity-stream",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["curiositystream"],
            "platforms": ["android_tv", "fire_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "curiositystream",
            "fire_tv": "curiositystream",
            "tvos": "curiositystream",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 79,
    "technical_name": "nbc",
    "short_name": "nbc",
    "clear_name": "NBC",
    "priority": 5,
    "display_priority": 51,
    "monetization_types": ["free", "flatrate"],
    "icon_url": "/icon/922526/{profile}",
    "icon_blur_hash": "KfQcVS-7_M$*smbu?wN_M{",
    "slug": "nbc",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 34,
    "technical_name": "epix",
    "short_name": "epx",
    "clear_name": "Epix",
    "priority": 5,
    "display_priority": 52,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/633230/{profile}",
    "icon_blur_hash": "LG7^-~of4nRioffQWBf601WB^+t7",
    "slug": "epix",
    "data": {
        "deeplink_data": [{
            "scheme": "tvwatchnext",
            "packages": ["com.epix.epix"],
            "platforms": ["android_tv"],
            "path_template": "epix/playback/%DEEPLINK%",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.epix.epix",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 211,
    "technical_name": "freeform",
    "short_name": "ffm",
    "clear_name": "Freeform",
    "priority": 5,
    "display_priority": 53,
    "monetization_types": ["free"],
    "icon_url": "/icon/79299282/{profile}",
    "icon_blur_hash": "KX2HhOfRirflfRfQirfkf4",
    "slug": "freeform",
    "data": {
        "deeplink_data": [{
            "scheme": "content://",
            "packages": ["com.disney.datg.videoplatforms.android.abcf"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "com.disney.datg.videoplatforms.android.abcf/view?deeplink=abcfamilyplayer://%DEEPLINK%\u0026requires_sign_in=false",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.disney.datg.videoplatforms.android.abcf",
            "fire_tv": "com.disney.datg.videoplatforms.android.abcf",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 155,
    "technical_name": "history",
    "short_name": "his",
    "clear_name": "History",
    "priority": 5,
    "display_priority": 54,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/6675367/{profile}",
    "icon_blur_hash": "KRB.[ba#0hngayS69xj[-S",
    "slug": "history",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["history"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "history",
            "fire_tv": "history",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 215,
    "technical_name": "syfy",
    "short_name": "sfy",
    "clear_name": "Syfy",
    "priority": 5,
    "display_priority": 55,
    "monetization_types": ["free"],
    "icon_url": "/icon/81397159/{profile}",
    "icon_blur_hash": "KU9HX1odRUt5fPaz4aWCt3",
    "slug": "syfy",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["syfy"],
            "platforms": ["tvos", "fire_tv", "android_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "syfy",
            "fire_tv": "syfy",
            "tvos": "syfy",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 156,
    "technical_name": "aetv",
    "short_name": "aae",
    "clear_name": "A\u0026E",
    "priority": 5,
    "display_priority": 56,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/6675987/{profile}",
    "icon_blur_hash": "KUB3.^ofIUofayWB00WB%M",
    "slug": "aande",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["aetv"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "aetv",
            "fire_tv": "aetv",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 157,
    "technical_name": "lifetime",
    "short_name": "lft",
    "clear_name": "Lifetime",
    "priority": 5,
    "display_priority": 57,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/6675988/{profile}",
    "icon_blur_hash": "KvN5i9n%1an*fQbH1GbHXS",
    "slug": "lifetime",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["lifetime"],
            "platforms": ["tvos", "android_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "lifetime",
            "fire_tv": "",
            "tvos": "lifetime",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 99,
    "technical_name": "shudder",
    "short_name": "shd",
    "clear_name": "Shudder",
    "priority": 5,
    "display_priority": 58,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/2562359/{profile}",
    "icon_blur_hash": "KB6iXAjt1]jtjta|6MfQ]U",
    "slug": "shudder",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 185,
    "technical_name": "screambox",
    "short_name": "scb",
    "clear_name": "Screambox",
    "priority": 5,
    "display_priority": 59,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/43432939/{profile}",
    "icon_blur_hash": "KC9@eex]M{%gofWB00RP%M",
    "slug": "screambox",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 87,
    "technical_name": "acorntv",
    "short_name": "act",
    "clear_name": "Acorn TV",
    "priority": 5,
    "display_priority": 60,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/151881328/{profile}",
    "icon_blur_hash": "K75X}]S%ITpMoMWA0Jr=xv",
    "slug": "acorn-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 143,
    "technical_name": "sundancenow",
    "short_name": "sdn",
    "clear_name": "Sundance Now",
    "priority": 5,
    "display_priority": 61,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/5676163/{profile}",
    "icon_blur_hash": "LfCO{Zj[0jWV${ayIuj[ItfQ$|js",
    "slug": "sundance-now",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 241,
    "technical_name": "popcornflix",
    "short_name": "pcf",
    "clear_name": "Popcornflix",
    "priority": 5,
    "display_priority": 62,
    "monetization_types": ["ads"],
    "icon_url": "/icon/109299737/{profile}",
    "icon_blur_hash": "KeShyMn%u5wJogbvy?ofVY",
    "slug": "popcornflix",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 151,
    "technical_name": "britbox",
    "short_name": "bbo",
    "clear_name": "BritBox",
    "priority": 5,
    "display_priority": 63,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/6368831/{profile}",
    "icon_blur_hash": "KB6{prtnIqcGj[f80PWC%0",
    "slug": "britbox",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 100,
    "technical_name": "guidedoc",
    "short_name": "gdc",
    "clear_name": "GuideDoc",
    "priority": 5,
    "display_priority": 64,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/2625277/{profile}",
    "icon_blur_hash": "KOB6fcS50NW?ayj[4=s:-n",
    "slug": "guidedoc",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 14,
    "technical_name": "realeyz",
    "short_name": "rlz",
    "clear_name": "realeyz",
    "priority": 5,
    "display_priority": 65,
    "monetization_types": ["flatrate", "rent", "buy"],
    "icon_url": "/icon/441358/{profile}",
    "icon_blur_hash": "KKSY~y?bxu?bj[j[~qofM{",
    "slug": "realeyz",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 11,
    "technical_name": "mubi",
    "short_name": "mbi",
    "clear_name": "Mubi",
    "priority": 11,
    "display_priority": 66,
    "monetization_types": ["flatrate", "rent"],
    "icon_url": "/icon/164970114/{profile}",
    "icon_blur_hash": "KF5rQpoiRib2axj@9DWCt7",
    "slug": "mubi",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["mubi"],
            "platforms": ["fire_tv", "android_tv", "tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "mubi",
            "fire_tv": "mubi",
            "tvos": "mubi",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 60,
    "technical_name": "fandango",
    "short_name": "fad",
    "clear_name": "Fandango",
    "priority": 6,
    "display_priority": 67,
    "monetization_types": ["cinema"],
    "icon_url": "/icon/811323/{profile}",
    "icon_blur_hash": "KaS=ejt,uPs:f6f+uOa0Z$",
    "slug": "fandango",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 175,
    "technical_name": "netflixkids",
    "short_name": "nfk",
    "clear_name": "Netflix Kids",
    "priority": 5,
    "display_priority": 68,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/14385750/{profile}",
    "icon_blur_hash": "KlRVnW%2%M?Hj@Rj.mkCRj",
    "slug": "netflix-kids",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.netflix.ninja", "com.netflix.mediaclient"],
            "platforms": ["fire_tv", "android_tv"],
            "path_template": "www.netflix.com/watch/%DEEPLINK%",
            "extras": {
                "S.source": "30"
            }
        }, {
            "scheme": "",
            "packages": ["surprice"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.netflix.ninja",
            "fire_tv": "com.netflix.ninja",
            "tvos": "surprice",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 247,
    "technical_name": "pantaya",
    "short_name": "pty",
    "clear_name": "Pantaya",
    "priority": 5,
    "display_priority": 69,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/120137063/{profile}",
    "icon_blur_hash": "KcEfKBx]01bwWBem4oRi-;",
    "slug": "pantaya",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 248,
    "technical_name": "boomerang",
    "short_name": "bmg",
    "clear_name": "Boomerang",
    "priority": 5,
    "display_priority": 70,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/120144009/{profile}",
    "icon_blur_hash": "KcQ]+wof-;t7j[WB~qj[Rj",
    "slug": "boomerang",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 251,
    "technical_name": "umc",
    "short_name": "umc",
    "clear_name": "Urban Movie Channel",
    "priority": 5,
    "display_priority": 71,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/121380910/{profile}",
    "icon_blur_hash": "KYBqoPkCMwyEa}V@01WBxt",
    "slug": "urban-movie-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 268,
    "technical_name": "historyvault",
    "short_name": "hvt",
    "clear_name": "History Vault",
    "priority": 5,
    "display_priority": 72,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124941612/{profile}",
    "icon_blur_hash": "KEATZhae0MI9oLXA0LkC-p",
    "slug": "history-vault",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 254,
    "technical_name": "dovechannel",
    "short_name": "dvc",
    "clear_name": "Dove Channel",
    "priority": 5,
    "display_priority": 73,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/123316731/{profile}",
    "icon_blur_hash": "KPErLz~AE3?as:IpIpE3Ip",
    "slug": "dove-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 255,
    "technical_name": "yupptv",
    "short_name": "ytv",
    "clear_name": "Yupp TV",
    "priority": 5,
    "display_priority": 74,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/123324312/{profile}",
    "icon_blur_hash": "KC84PYb_M{jZayj@0LROxu",
    "slug": "yupp-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 218,
    "technical_name": "erosnow",
    "short_name": "ern",
    "clear_name": "Eros Now",
    "priority": 5,
    "display_priority": 75,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/82869265/{profile}",
    "icon_blur_hash": "KhEz+;t7Rkogj[j[02WBt6",
    "slug": "eros-now",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 259,
    "technical_name": "magnoliaselects",
    "short_name": "mns",
    "clear_name": "Magnolia Selects",
    "priority": 5,
    "display_priority": 76,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124651823/{profile}",
    "icon_blur_hash": "KH3XFoqFWVlTkCf6MxWVkC",
    "slug": "magnolia-selects",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 260,
    "technical_name": "wwenetwork",
    "short_name": "wwe",
    "clear_name": "WWE Network",
    "priority": 5,
    "display_priority": 77,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124657456/{profile}",
    "icon_blur_hash": "KMD01Abb00x]jts:0KjZ?b",
    "slug": "wwe-network",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 264,
    "technical_name": "myoutdoortv",
    "short_name": "mot",
    "clear_name": "MyOutdoorTV",
    "priority": 5,
    "display_priority": 78,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/124832728/{profile}",
    "icon_blur_hash": "KQB3mJxaS#xuj[ay0KRjr?",
    "slug": "myoutdoortv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 261,
    "technical_name": "amazonnickhits",
    "short_name": "anh",
    "clear_name": "Nickhits Amazon Channel",
    "priority": 5,
    "display_priority": 79,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124867882/{profile}",
    "icon_blur_hash": "KvL=NnrwoyA6sARj-;Sxog",
    "slug": "nickhits-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 262,
    "technical_name": "amazonnoggin",
    "short_name": "ang",
    "clear_name": "Noggin Amazon Channel",
    "priority": 5,
    "display_priority": 80,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124867843/{profile}",
    "icon_blur_hash": "KnIstDkCO[0}oII[-UafxG",
    "slug": "noggin-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 267,
    "technical_name": "hopster",
    "short_name": "htv",
    "clear_name": "Hopster TV",
    "priority": 5,
    "display_priority": 81,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124941576/{profile}",
    "icon_blur_hash": "KkSrWRt7pHnikVozy=kBaL",
    "slug": "hopster-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 275,
    "technical_name": "laughoutloud",
    "short_name": "lol",
    "clear_name": "Laugh Out Loud",
    "priority": 5,
    "display_priority": 82,
    "monetization_types": ["ads"],
    "icon_url": "/icon/127412694/{profile}",
    "icon_blur_hash": "KTH=dFoxySoxj@fQ*Dj@Q=",
    "slug": "laugh-out-loud",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 276,
    "technical_name": "smithsonianchannel",
    "short_name": "ssc",
    "clear_name": "Smithsonian Channel",
    "priority": 5,
    "display_priority": 83,
    "monetization_types": ["free", "flatrate"],
    "icon_url": "/icon/127413290/{profile}",
    "icon_blur_hash": "KhOdFPxaD4xaj[aeD4ae%#",
    "slug": "smithsonian-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 278,
    "technical_name": "pureflix",
    "short_name": "pux",
    "clear_name": "Pure Flix",
    "priority": 5,
    "display_priority": 84,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/127414508/{profile}",
    "icon_blur_hash": "KH9Ii8n2H;4;b_wtDhWBo#",
    "slug": "pure-flix",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 281,
    "technical_name": "hallmarkmovies",
    "short_name": "hmm",
    "clear_name": "Hallmark Movies",
    "priority": 5,
    "display_priority": 85,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/127435091/{profile}",
    "icon_blur_hash": "KJFre;t74U4UWBX8IUays:",
    "slug": "hallmark-movies",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 284,
    "technical_name": "lifetimemovieclub",
    "short_name": "lmc",
    "clear_name": "Lifetime Movie Club",
    "priority": 5,
    "display_priority": 86,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/127573922/{profile}",
    "icon_blur_hash": "KrG%4VbH0{#-jaNt9[o1-V",
    "slug": "lifetime-movie-club",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 293,
    "technical_name": "amazonpbskids",
    "short_name": "apk",
    "clear_name": "PBS Kids Amazon Channel",
    "priority": 5,
    "display_priority": 87,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003624/{profile}",
    "icon_blur_hash": "KnP??eWo_4o[jYxvs;a#Rj",
    "slug": "pbs-kids-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 288,
    "technical_name": "amazonboomerang",
    "short_name": "abo",
    "clear_name": "Boomerang Amazon Channel",
    "priority": 5,
    "display_priority": 88,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003323/{profile}",
    "icon_blur_hash": "KMP%IqX9~qbws.Rj_3azM{",
    "slug": "boomerang-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 289,
    "technical_name": "amazoncinemax",
    "short_name": "acn",
    "clear_name": "Cinemax Amazon Channel",
    "priority": 5,
    "display_priority": 89,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003398/{profile}",
    "icon_blur_hash": "KmHB[0kDt701e.RP-;WBof",
    "slug": "cinemax-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 292,
    "technical_name": "amazonpantaya",
    "short_name": "apa",
    "clear_name": "Pantaya Amazon Channel",
    "priority": 5,
    "display_priority": 90,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003609/{profile}",
    "icon_blur_hash": "KzIX{jbcRk00oJM{%gaeof",
    "slug": "pantaya-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 290,
    "technical_name": "amazonhallmarkmoviesnow",
    "short_name": "ahm",
    "clear_name": "Hallmark Movies Now Amazon Channel",
    "priority": 5,
    "display_priority": 91,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003434/{profile}",
    "icon_blur_hash": "KtI54BWYoz02o0Ri-:fkof",
    "slug": "hallmark-movies-now-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 294,
    "technical_name": "amazonpbsmasterpiece",
    "short_name": "apm",
    "clear_name": "PBS Masterpiece Amazon Channel",
    "priority": 5,
    "display_priority": 92,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003785/{profile}",
    "icon_blur_hash": "KNOgE64.xuE1ROMx~qxuxu",
    "slug": "pbs-masterpiece-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 295,
    "technical_name": "amazonviewster",
    "short_name": "avt",
    "clear_name": "Viewster Amazon Channel",
    "priority": 5,
    "display_priority": 93,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003797/{profile}",
    "icon_blur_hash": "KsQ*f7bIr=v0oKRPtmayog",
    "slug": "viewster-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 291,
    "technical_name": "amazonmzchoice",
    "short_name": "ame",
    "clear_name": "MZ Choice Amazon Channel",
    "priority": 5,
    "display_priority": 94,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131003470/{profile}",
    "icon_blur_hash": "KzI;k*flJQ4ToKIo-;ayxa",
    "slug": "mz-choice-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 299,
    "technical_name": "slingtv",
    "short_name": "stv",
    "clear_name": "Sling TV",
    "priority": 5,
    "display_priority": 95,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131739403/{profile}",
    "icon_blur_hash": "KF2s%;aeV@ofj]WBQkj[kD",
    "slug": "sling-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 300,
    "technical_name": "plutotv",
    "short_name": "ptv",
    "clear_name": "Pluto TV",
    "priority": 5,
    "display_priority": 96,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/131862988/{profile}",
    "icon_blur_hash": "KMBg3uj[00j[fQ%M4nay-;",
    "slug": "pluto-tv",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["com.pluto"],
            "platforms": ["tvos", "android_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.pluto",
            "fire_tv": "",
            "tvos": "com.pluto",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 200,
    "technical_name": "amazonhbonow",
    "short_name": "ahb",
    "clear_name": "HBO Now Amazon Channel",
    "priority": 5,
    "display_priority": 97,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944793/{profile}",
    "icon_blur_hash": "KdJ*n[WYIU0KoJD%~qWVj[",
    "slug": "hbo-now-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 203,
    "technical_name": "amazonshowtime",
    "short_name": "ash",
    "clear_name": "Showtime Amazon Channel",
    "priority": 5,
    "display_priority": 98,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944796/{profile}",
    "icon_blur_hash": "KjN]34bIMdD4oJIAyDaytR",
    "slug": "showtime-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 198,
    "technical_name": "amazoncbsallaccess",
    "short_name": "acb",
    "clear_name": "CBS All Access Amazon Channel",
    "priority": 5,
    "display_priority": 99,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944791/{profile}",
    "icon_blur_hash": "KiKL%pbb5F0=odD+-PWV%K",
    "slug": "cbs-all-access-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 438,
    "technical_name": "chaiflicks",
    "short_name": "chf",
    "clear_name": "Chai Flicks",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/201708663/{profile}",
    "icon_blur_hash": "KWF=XGxZ0qt6j[WX0qWExV",
    "slug": "chai-flicks",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 194,
    "technical_name": "amazonstarzplay",
    "short_name": "ast",
    "clear_name": "Starz Play Amazon Channel",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/68363608/{profile}",
    "icon_blur_hash": "KqGuwKbIkD00f5RP-;azt7",
    "slug": "starz-play-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }, {
            "scheme": "",
            "packages": ["starz"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "starz",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 470,
    "technical_name": "filmdetective",
    "short_name": "tfd",
    "clear_name": "The Film Detective",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/207967267/{profile}",
    "icon_blur_hash": "LN9%xFay00ofRPayt7j[D%j[%MWB",
    "slug": "the-film-detective",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 464,
    "technical_name": "kocowa",
    "short_name": "koc",
    "clear_name": "Kocowa",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/207331738/{profile}",
    "icon_blur_hash": "KF8E3xxvRkxvogf900WCt7",
    "slug": "kocowa",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 459,
    "technical_name": "netflixfree",
    "short_name": "nff",
    "clear_name": "Netflix Free",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["free"],
    "icon_url": "/icon/207468065/{profile}",
    "icon_blur_hash": "KYEKD$Mx2D-pRPRjFwofrD",
    "slug": "netflix-free",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 458,
    "technical_name": "vicetv",
    "short_name": "vtv",
    "clear_name": "Vice TV ",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["ads"],
    "icon_url": "/icon/202929812/{profile}",
    "icon_blur_hash": "KJSY{qt7%MxufQfQ~qofM{",
    "slug": "vice-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 457,
    "technical_name": "vixtv",
    "short_name": "vix",
    "clear_name": "VIX ",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["ads"],
    "icon_url": "/icon/202926990/{profile}",
    "icon_blur_hash": "KI9QmxayIUofj[ay00j]%M",
    "slug": "vix",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 455,
    "technical_name": "nightflightplus",
    "short_name": "nfp",
    "clear_name": "Night Flight Plus",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/202924365/{profile}",
    "icon_blur_hash": "KC7^xot69#xbj@WX0LRk$_",
    "slug": "night-flight-plus",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 454,
    "technical_name": "topic",
    "short_name": "tpc",
    "clear_name": "Topic",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/202921120/{profile}",
    "icon_blur_hash": "KI8E6$j[D%ayfQj[00ay-;",
    "slug": "topic",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 453,
    "technical_name": "mtv",
    "short_name": "mtv",
    "clear_name": "MTV",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["ads"],
    "icon_url": "/icon/202918396/{profile}",
    "icon_blur_hash": "KuO|b:od-%t5j@od~ft4Ia",
    "slug": "mtv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 446,
    "technical_name": "retrocrush",
    "short_name": "rtc",
    "clear_name": "Retrocrush",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate", "ads", "free"],
    "icon_url": "/icon/202068535/{profile}",
    "icon_blur_hash": "KOA0zNayD*W;ayjZ01j[%L",
    "slug": "retrocrush",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 445,
    "technical_name": "classixapp",
    "short_name": "cla",
    "clear_name": "Classix",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/202068388/{profile}",
    "icon_blur_hash": "KI9H2_t7M{t7j[j[00WBxu",
    "slug": "classix",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 427,
    "technical_name": "mhzchoice",
    "short_name": "mhz",
    "clear_name": "Mhz Choice",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["buy", "flatrate"],
    "icon_url": "/icon/200839772/{profile}",
    "icon_blur_hash": "KaLd_$t7Mdt7kCay8wWBx]",
    "slug": "mhz-choice",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 430,
    "technical_name": "hidive",
    "short_name": "hdv",
    "clear_name": "HiDive",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/201071617/{profile}",
    "icon_blur_hash": "L@3e+nfkVBf6j[fQazfQW?fQoJfQ",
    "slug": "hidive",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 444,
    "technical_name": "dekkoo",
    "short_name": "dkk",
    "clear_name": "Dekkoo",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/202068207/{profile}",
    "icon_blur_hash": "KHQm0MoM?^{xo0O?.mt7R5",
    "slug": "dekkoo",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 433,
    "technical_name": "ovid",
    "short_name": "ovi",
    "clear_name": "OVID",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/201697454/{profile}",
    "icon_blur_hash": "KNS$li%hkZ-:flj].AxBaI",
    "slug": "ovid",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 439,
    "technical_name": "shoutfactorytv",
    "short_name": "sft",
    "clear_name": "Shout! Factory TV",
    "priority": 5,
    "display_priority": 100,
    "monetization_types": ["free"],
    "icon_url": "/icon/201913784/{profile}",
    "icon_blur_hash": "KWCZbEj[9Ft7j[WB00WB-;",
    "slug": "shout-factory-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 204,
    "technical_name": "amazonshudder",
    "short_name": "asd",
    "clear_name": "Shudder Amazon Channel",
    "priority": 5,
    "display_priority": 101,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944797/{profile}",
    "icon_blur_hash": "KvG*?0kCT04TjYNG-;a}s:",
    "slug": "shudder-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }, {
            "scheme": "",
            "packages": ["nicenstein"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "nicenstein",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 201,
    "technical_name": "amazonmubi",
    "short_name": "amu",
    "clear_name": "Mubi Amazon Channel",
    "priority": 5,
    "display_priority": 102,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944794/{profile}",
    "icon_blur_hash": "KvI5-.bIof0KoKRj-qayof",
    "slug": "mubi-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }, {
            "scheme": "",
            "packages": ["gut"],
            "platforms": ["tvos"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "gut",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 196,
    "technical_name": "amazonacorntv",
    "short_name": "aac",
    "clear_name": "AcornTV Amazon Channel",
    "priority": 5,
    "display_priority": 103,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944789/{profile}",
    "icon_blur_hash": "LZC~@-a}0KoJ4njs-;WX-;ayMxoL",
    "slug": "acorntv-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 197,
    "technical_name": "amazonbritbox",
    "short_name": "abb",
    "clear_name": "BritBox Amazon Channel",
    "priority": 5,
    "display_priority": 104,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944790/{profile}",
    "icon_blur_hash": "KsGUIBbwoz1RkCRj-9f6of",
    "slug": "britbox-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 199,
    "technical_name": "amazonfandor",
    "short_name": "afa",
    "clear_name": "Fandor Amazon Channel",
    "priority": 5,
    "display_priority": 105,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944792/{profile}",
    "icon_blur_hash": "K*HxscWXRj00oJRP-;ayt7",
    "slug": "fandor-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 202,
    "technical_name": "amazonscreambox",
    "short_name": "asb",
    "clear_name": "Screambox Amazon Channel",
    "priority": 5,
    "display_priority": 106,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944795/{profile}",
    "icon_blur_hash": "KzHoB=bIWV00oJRP-;WVt7",
    "slug": "screambox-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 205,
    "technical_name": "amazonsundancenow",
    "short_name": "asn",
    "clear_name": "Sundance Now Amazon Channel",
    "priority": 5,
    "display_priority": 107,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/73944798/{profile}",
    "icon_blur_hash": "K#Jao@bII]4To0Ip%gWCt6",
    "slug": "sundance-now-amazon-channel",
    "data": {
        "deeplink_data": [{
            "scheme": "https",
            "packages": ["com.amazon.avod"],
            "platforms": ["fire_tv"],
            "path_template": "watch.amazon.de/watch?asin=%DEEPLINK%\u0026contentType=MOVIE\u0026territory=DE\u0026ref_=atv_dp_pb_core",
            "extras": null
        }, {
            "scheme": "",
            "packages": ["com.amazon.amazonvideo.livingroom"],
            "platforms": ["android_tv"],
            "path_template": "",
            "extras": {
                "S.com.amazon.ignition.DeepLinkIntent.DEEP_LINK": "%DEEPLINK%"
            }
        }],
        "packages": {
            "android_tv": "com.amazon.amazonvideo.livingroom",
            "fire_tv": "com.amazon.avod",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 317,
    "technical_name": "cartoonnetwork",
    "short_name": "ctw",
    "clear_name": "Cartoon Network",
    "priority": 5,
    "display_priority": 108,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/141257713/{profile}",
    "icon_blur_hash": "LG8qNgxu00IUt7j[RjWB4nM{?b%M",
    "slug": "cartoon-network",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 318,
    "technical_name": "adultswim",
    "short_name": "ads",
    "clear_name": "Adult Swim",
    "priority": 5,
    "display_priority": 109,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/141261090/{profile}",
    "icon_blur_hash": "KYDmEpf6M{kCkCV@00ayxu",
    "slug": "adult-swim",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 322,
    "technical_name": "usanetwork",
    "short_name": "usn",
    "clear_name": "USA Network",
    "priority": 5,
    "display_priority": 110,
    "monetization_types": ["free", "flatrate"],
    "icon_url": "/icon/141752011/{profile}",
    "icon_blur_hash": "KLO*;P=0R5-An%ayL#aftl",
    "slug": "usa-network",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 328,
    "technical_name": "foxusa",
    "short_name": "fus",
    "clear_name": "Fox",
    "priority": 5,
    "display_priority": 111,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/144006145/{profile}",
    "icon_blur_hash": "KfF@.SkD4{ofj@az0Vj?%F",
    "slug": "fox",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["foxusa"],
            "platforms": ["tvos", "android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "foxusa",
            "fire_tv": "foxusa",
            "tvos": "foxusa",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 331,
    "technical_name": "flixfling",
    "short_name": "fxf",
    "clear_name": "FlixFling",
    "priority": 5,
    "display_priority": 112,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/147271109/{profile}",
    "icon_blur_hash": "KVNu8Y$#xDs.WWWV}-j@R-",
    "slug": "flixfling",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["flixfling"],
            "platforms": ["android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "flixfling",
            "fire_tv": "flixfling",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 343,
    "technical_name": "amazonbetplus",
    "short_name": "bpc",
    "clear_name": "Bet+ Amazon Channel",
    "priority": 5,
    "display_priority": 113,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/149271019/{profile}",
    "icon_blur_hash": "K,IEIva%bv4UoMRj%gWUof",
    "slug": "bet-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 344,
    "technical_name": "viki",
    "short_name": "vik",
    "clear_name": "Rakuten Viki",
    "priority": 5,
    "display_priority": 114,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/149297567/{profile}",
    "icon_blur_hash": "KRCc5#g4D?t8WWbE0XoK${",
    "slug": "rakuten-viki",
    "data": {
        "deeplink_data": [{
            "scheme": "",
            "packages": ["viki"],
            "platforms": ["tvos", "android_tv", "fire_tv"],
            "path_template": "",
            "extras": null
        }],
        "packages": {
            "android_tv": "viki",
            "fire_tv": "viki",
            "tvos": "viki",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 352,
    "technical_name": "amctheatresondemand",
    "short_name": "amo",
    "clear_name": "AMC on Demand",
    "priority": 5,
    "display_priority": 115,
    "monetization_types": ["rent", "buy"],
    "icon_url": "/icon/166008757/{profile}",
    "icon_blur_hash": "K86ZewWUS1avj]Wq1Vj]sp",
    "slug": "amc-on-demand",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 355,
    "technical_name": "darkmatter",
    "short_name": "dkm",
    "clear_name": "Darkmatter TV",
    "priority": 5,
    "display_priority": 116,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/155553229/{profile}",
    "icon_blur_hash": "KGJYIR0iTD?D0$s:JTJ,eB",
    "slug": "darkmatter-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 361,
    "technical_name": "tcm",
    "short_name": "tcm",
    "clear_name": "TCM",
    "priority": 5,
    "display_priority": 117,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/162036726/{profile}",
    "icon_blur_hash": "KZO3|Tofxu%Mj[af~qj[IU",
    "slug": "tcm",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 365,
    "technical_name": "bravotv",
    "short_name": "brv",
    "clear_name": "Bravo TV",
    "priority": 5,
    "display_priority": 118,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/164156069/{profile}",
    "icon_blur_hash": "KXNLY~kC=}Sej[s:}^oLEK",
    "slug": "bravo-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 363,
    "technical_name": "tnt",
    "short_name": "tnt",
    "clear_name": "TNT",
    "priority": 5,
    "display_priority": 119,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/164147684/{profile}",
    "icon_blur_hash": "KYI~74%g01x]ogWBM|o#IU",
    "slug": "tnt",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 366,
    "technical_name": "foodnetwork",
    "short_name": "fnw",
    "clear_name": "Food Network",
    "priority": 5,
    "display_priority": 120,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/165029433/{profile}",
    "icon_blur_hash": "KsJ#Z:of1HoLf6a}0yayxb",
    "slug": "food-network",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 397,
    "technical_name": "bbcamerica",
    "short_name": "bca",
    "clear_name": "BBC America",
    "priority": 5,
    "display_priority": 121,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/198412956/{profile}",
    "icon_blur_hash": "LjET;.jt0|a|ozfQWBfkMxfQx]fQ",
    "slug": "bbc-america",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 368,
    "technical_name": "indieflix",
    "short_name": "ind",
    "clear_name": "IndieFlix",
    "priority": 5,
    "display_priority": 122,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/165029440/{profile}",
    "icon_blur_hash": "KA5}N]pIIoadayfk0eVY%2",
    "slug": "indieflix",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 398,
    "technical_name": "ahctv",
    "short_name": "ahc",
    "clear_name": "AHCTV",
    "priority": 5,
    "display_priority": 123,
    "monetization_types": ["free", "flatrate"],
    "icon_url": "/icon/198433889/{profile}",
    "icon_blur_hash": "KwQ]mAof%Mo0j[oz_NofM{",
    "slug": "ahctv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 412,
    "technical_name": "tlc",
    "short_name": "tlc",
    "clear_name": "TLC",
    "priority": 5,
    "display_priority": 124,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433923/{profile}",
    "icon_blur_hash": "K=SE,ls:o}s:fkfky?j[Vs",
    "slug": "tlc",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 406,
    "technical_name": "hgtv",
    "short_name": "hgt",
    "clear_name": "HGTV",
    "priority": 5,
    "display_priority": 125,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433905/{profile}",
    "icon_blur_hash": "KYPX9D-pIU~CP9VsDiVsTc",
    "slug": "hgtv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 405,
    "technical_name": "diynetwork",
    "short_name": "diy",
    "clear_name": "DIY Network",
    "priority": 5,
    "display_priority": 126,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433903/{profile}",
    "icon_blur_hash": "KTBOZrtQ9FtQayWB00Rj-;",
    "slug": "diy-network",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 408,
    "technical_name": "investigationdiscovery",
    "short_name": "inv",
    "clear_name": "Investigation Discovery",
    "priority": 5,
    "display_priority": 127,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433912/{profile}",
    "icon_blur_hash": "KkEJiBbuOWkVoff*2YbIso",
    "slug": "investigation-discovery",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 411,
    "technical_name": "sciencechannel",
    "short_name": "sci",
    "clear_name": "Science Channel",
    "priority": 5,
    "display_priority": 128,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433922/{profile}",
    "icon_blur_hash": "K#Ju7al7@X%1k9r?MKjYkq",
    "slug": "science-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 402,
    "technical_name": "destinationamerica",
    "short_name": "dea",
    "clear_name": "Destination America",
    "priority": 5,
    "display_priority": 129,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433897/{profile}",
    "icon_blur_hash": "KXC6$2WB4nxuofRj00of?b",
    "slug": "destination-america",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 399,
    "technical_name": "animalplanet",
    "short_name": "apl",
    "clear_name": "Animal Planet",
    "priority": 5,
    "display_priority": 130,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433891/{profile}",
    "icon_blur_hash": "KOPG:StA%1Ow4@Rl?aN20P",
    "slug": "animal-planet",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 404,
    "technical_name": "discoverylife",
    "short_name": "dil",
    "clear_name": "Discovery Life",
    "priority": 5,
    "display_priority": 131,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433900/{profile}",
    "icon_blur_hash": "KZC%2Xa#D%a~ofay00of%L",
    "slug": "discovery-life",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 403,
    "technical_name": "discoverygo",
    "short_name": "dis",
    "clear_name": "Discovery",
    "priority": 5,
    "display_priority": 132,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198441211/{profile}",
    "icon_blur_hash": "KWDLmAtSD*NyWVoL0LR*xa",
    "slug": "discovery",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 410,
    "technical_name": "motortrend",
    "short_name": "mtr",
    "clear_name": "Motor Trend",
    "priority": 5,
    "display_priority": 133,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433920/{profile}",
    "icon_blur_hash": "KjQ@J1UvQmQ8yXayZhR5t,",
    "slug": "motor-trend",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 400,
    "technical_name": "cookingchanneltv",
    "short_name": "coo",
    "clear_name": "Cooking Channel",
    "priority": 5,
    "display_priority": 134,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433892/{profile}",
    "icon_blur_hash": "KKAUZ#k9M|ogf8ay01ahtQ",
    "slug": "cooking-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 413,
    "technical_name": "travelchannel",
    "short_name": "tra",
    "clear_name": "Travel Channel",
    "priority": 5,
    "display_priority": 135,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/198433924/{profile}",
    "icon_blur_hash": "KK96]YxoIst5ocoL0hRo$}",
    "slug": "travel-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 418,
    "technical_name": "paramountnetwork",
    "short_name": "pnw",
    "clear_name": "Paramount Network",
    "priority": 5,
    "display_priority": 136,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/199216643/{profile}",
    "icon_blur_hash": "KWD-R9WY02%MoLM|02of-o",
    "slug": "paramount-network",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 417,
    "technical_name": "heretv",
    "short_name": "hrv",
    "clear_name": "Here TV",
    "priority": 5,
    "display_priority": 137,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/199216642/{profile}",
    "icon_blur_hash": "K64dWTsqS0oMa{a{1VS0sq",
    "slug": "here-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 419,
    "technical_name": "tvland",
    "short_name": "tvl",
    "clear_name": "TV Land",
    "priority": 5,
    "display_priority": 138,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/199382332/{profile}",
    "icon_blur_hash": "KZSd%Nt7a1spf6fkUbenkW",
    "slug": "tv-land",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 420,
    "technical_name": "logotv",
    "short_name": "ltv",
    "clear_name": "Logo TV",
    "priority": 5,
    "display_priority": 139,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/199382333/{profile}",
    "icon_blur_hash": "KrL.4rs+M}:gf}n,OiWrt6",
    "slug": "logo-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 422,
    "technical_name": "vh1",
    "short_name": "vho",
    "clear_name": "VH1",
    "priority": 5,
    "display_priority": 140,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/199382338/{profile}",
    "icon_blur_hash": "KYBDWpfQIUofj[ay00fQ%M",
    "slug": "vh1",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 263,
    "technical_name": "amazondreamworkstv",
    "short_name": "adw",
    "clear_name": "DreamWorksTV Amazon Channel",
    "priority": 5,
    "display_priority": 200,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/124868001/{profile}",
    "icon_blur_hash": "KmJ,ELT0Ot1*s-Ip-orqwv",
    "slug": "dreamworkstv-amazon-channel",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 432,
    "technical_name": "flixpremiere",
    "short_name": "fpm",
    "clear_name": "Flix Premiere",
    "priority": 5,
    "display_priority": 200,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/201076273/{profile}",
    "icon_blur_hash": "KmQ9_@ayxu%Mj[Rj~qofWB",
    "slug": "flix-premiere",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 507,
    "technical_name": "trutv",
    "short_name": "tru",
    "clear_name": "tru TV",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/209564918/{profile}",
    "icon_blur_hash": "KbJ]=_j@owodkAWD?,kAV^",
    "slug": "tru-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 508,
    "technical_name": "disneynow",
    "short_name": "dnw",
    "clear_name": "DisneyNOW",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/209565063/{profile}",
    "icon_blur_hash": "L=H[,TbH1Gn%j[f6ayfkVsf6o}fk",
    "slug": "disneynow",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 509,
    "technical_name": "wetv",
    "short_name": "wet",
    "clear_name": "WeTV",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/209565207/{profile}",
    "icon_blur_hash": "Ka8~.}ayVrj]f6kCMHf6kD",
    "slug": "wetv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 514,
    "technical_name": "asiancrush",
    "short_name": "asc",
    "clear_name": "AsianCrush",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["ads"],
    "icon_url": "/icon/209565865/{profile}",
    "icon_blur_hash": "KmS:W~r;mQy9j?bFm6j[b_",
    "slug": "asiancrush",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 471,
    "technical_name": "filmrise",
    "short_name": "flr",
    "clear_name": "FILMRISE",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["ads", "free"],
    "icon_url": "/icon/207967415/{profile}",
    "icon_blur_hash": "LmF3AZjZ1HbHFda|#-jtRPfkt7ay",
    "slug": "filmrise",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 473,
    "technical_name": "revry",
    "short_name": "rvy",
    "clear_name": "Revry",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/208144706/{profile}",
    "icon_blur_hash": "KI8Mans;NYspjaa{0_R*xH",
    "slug": "revry",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 475,
    "technical_name": "docsville",
    "short_name": "dsv",
    "clear_name": "DOCSVILLE",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208148387/{profile}",
    "icon_blur_hash": "KFS=u7xuuPxuj[bHuOaenj",
    "slug": "docsville",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 485,
    "technical_name": "roosterteeth",
    "short_name": "rst",
    "clear_name": "Rooster Teeth",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["ads", "flatrate"],
    "icon_url": "/icon/208856114/{profile}",
    "icon_blur_hash": "KiL{0:?HW==|xuoz8wVsbF",
    "slug": "rooster-teeth",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 486,
    "technical_name": "spectrumondemand",
    "short_name": "sod",
    "clear_name": "Spectrum On Demand",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208857343/{profile}",
    "icon_blur_hash": "KL8#Q#j]00kEe-a#00j?_3",
    "slug": "spectrum-on-demand",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 487,
    "technical_name": "oxygen",
    "short_name": "oxy",
    "clear_name": "OXYGEN",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/208859185/{profile}",
    "icon_blur_hash": "KOA^Qft507t5t5WD07WV?W",
    "slug": "oxygen",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 495,
    "technical_name": "cgoodtv",
    "short_name": "cgt",
    "clear_name": "CGood TV",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["free"],
    "icon_url": "/icon/208864843/{profile}",
    "icon_blur_hash": "KARbbt-_hg-wjbnOhfnOaK",
    "slug": "cgood-tv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 503,
    "technical_name": "hiyah",
    "short_name": "hyh",
    "clear_name": "Hi-YAH",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate", "free"],
    "icon_url": "/icon/209563206/{profile}",
    "icon_blur_hash": "KLQgtenirXt7f6f6.nofTJ",
    "slug": "hi-yah",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 504,
    "technical_name": "vrvco",
    "short_name": "vrv",
    "clear_name": "VRV",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate", "ads"],
    "icon_url": "/icon/209564574/{profile}",
    "icon_blur_hash": "KjN0L7j[}~M{ayxu}~j?o#",
    "slug": "vrv",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}, {
    "id": 506,
    "technical_name": "tbs",
    "short_name": "tus",
    "clear_name": "TBS",
    "priority": 5,
    "display_priority": 1000,
    "monetization_types": ["flatrate"],
    "icon_url": "/icon/209564793/{profile}",
    "icon_blur_hash": "KcN9CF}?XfozkCWB4T=wob",
    "slug": "tbs",
    "data": {
        "deeplink_data": [],
        "packages": {
            "android_tv": "",
            "fire_tv": "",
            "tvos": "",
            "tizenos": "",
            "webos": ""
        }
    }
}]