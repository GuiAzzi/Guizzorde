...
import { snmAdminCommand } from './index.js';
				...
				if (snmSchedule) {
					...
					SNMSchedulesArray.set(this.guildId, {
						cronNew: new cron.CronJob(
							this.schedule.new,
							async () => {
								console.log(`Running CronJob "snmNew" at ${this.guildId}`);
								await snmAdminCommand.handler(
									new ChatInputCommandInteraction(client, {
						...
						cronStart: new cron.CronJob(
							this.schedule.start,
							async () => {
								console.log(`Running CronJob "snmStart" at ${this.guildId}`);
								await snmAdminCommand.handler(
									new ChatInputCommandInteraction(client, {
										guild_id: this.guildId,
										channel_id: SNMServerArray.get(this.guildId).defaultChannel,
						...
						cronEnd: new cron.CronJob(
							this.schedule.end,
							async () => {
								console.log(`Running CronJob "snmEnd" at ${this.guildId}`);
								await snmAdminCommand.handler(
									new ChatInputCommandInteraction(client, {
										guild_id: this.guildId,
										channel_id: SNMServerArray.get(this.guildId).defaultChannel,
