...
import { SNMSchedulesArray, SNMServer, SNMWeek } from "./index.js";
...
export const snmCommands = {
  ...
  snm: new GuizzordeCommand({
    ...
    handler: async function (interaction) {
      ...
      const snmWeekEmbed = new Discord.MessageEmbed()
        .setTitle("Searching...")
        .setColor(0x3498db);
      ...
      try {
        ...
        if (week != null && week <= 0) {
          ...
          for (let i = 0; i < arr.length; i++) {
            if (i === 0) {
              await client.api
                .webhooks(configObj.appId, interaction.token)
                .messages("@original")
                .patch({
                  data: {
                    embeds: [
  ...
  snmAdmin: new GuizzordeCommand({
    ...
    handler: async function (interaction, fromScheduler) {
      ...
      const memberPerformed = await guildPerformed.members.fetch(
      ...
      switch (command) {
        case "new": {
          try {
            ...
            if (
              !memberPerformed.permissions.has(
            ...
            const newSNMEmbed = new Discord.MessageEmbed()
        ...
        case "start": {
          try {
            ...
            // Interaction first contact (to be edited)
            const startSNMEmbed = new Discord.MessageEmbed()
            ...
            let voteMessage;
            if (!fromScheduler) {
              const webhookMessage = await client.api
                .webhooks(configObj.appId, interaction.token)
                .messages("@original")
                .patch({ data: {} });
              voteMessage = new Discord.Message(
                client,
                webhookMessage,
                client.channels.cache.get(webhookMessage.channel_id)
              );
            } else {
            ...
            const printArray = [];
            const emojiArray = client.guilds.cache.get(interaction.guildId)
              .emojis.cache;
            const emojisUsed = [];
        ...
        case "end": {
          try {
            ...
            // Interaction first contact (to be edited)
            const endSNMEmbed = new Discord.MessageEmbed()
              .setTitle("Ending SNM")
              .setDescription("🛠 Working...")
              .setColor(0x3498db);
            if (!fromScheduler) {
            ...
            if (lastSNMVoteMessage) {
              for (const actionRow of lastSNMVoteMessage.components) {
                for (const button of actionRow.components) {
                  if (button.type === "BUTTON") button.setDisabled("true");
                }
              }
              lastSNMVoteMessage.embeds[0].setFooter("This voting has ended");
              lastSNMVoteMessage.edit({
                embeds: lastSNMVoteMessage.embeds,
                components: lastSNMVoteMessage.components,
              });
            }
        ...
        switch (interactionOptions) {
          ...
          case "remove": {
            ...
            for (const userIndex in lastSNM.users) {
              for (const movieIndex in lastSNM.users[userIndex].movies) {
                if (
                  lastSNM.users[userIndex].movies[movieIndex].title ===
                  titleName
                ) {
                  stringFound = true;
                  // Checks if movie found was submitted by message author
                  if (
                    lastSNM.users[userIndex].userId ===
                    interaction.member.user.id
                  ) {
  ...
  snmRate: new GuizzordeCommand({
    ...
    handler: async function (interaction) {
      try {
        ...
        const lastFinishedSNM = await getSNMWeek(
          interaction.guildId,
          null,
          "finished"
        );
        ...
        let userObject = lastFinishedSNM.users.find(
        ...
        if (!userObject) {
          userObject =
            lastFinishedSNM.users[
              lastFinishedSNM.users.push({
            ...
            moviesVoted.push(
              `\`${
                lastSNM.users
                  .find((user) =>
                    user.movies.find(
                      (movie) => movie.titleKey === movieTitleKey
                    )
                  )
                  .movies.find((movie) => movie.titleKey === movieTitleKey)
                  .title
              }\``
            );
          });
